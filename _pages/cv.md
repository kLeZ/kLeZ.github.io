...
{% capture card10 %}
Sviluppo e manutenzione di **MyTicketSolution**, piattaforma multi-tenant utilizzata da vettori che offrono servizio di corriera (autobus).  
La piattaforma comprende un portale online pubblico per l'acquisto di biglietti, un portale di backoffice con autenticazione per la gestione della piattaforma da parte dei vettori, dei servizi web a corredo del funzionamento e due diverse **App** (1.0 con UWP e 2.0 con Xamarin) per la gestione delle corse utilizzate dagli autisti.

Analogamente, sviluppo e manutenzione di **Arriva Ticket**, piattaforma multi-tenant simile a MyTicketSolution.

Sviluppo e manutenzione di **Monster**, applicazione **WPF** che aiuta gli operatori della casa farmaceutica **Baxter** con il lavoro di produzione di medicinali tramite una lista di operazioni standard per lotto. L'applicazione viene utilizzata tramite comandi vocali e utilizza un fotocamera Reflex per la cattura delle immagini dei passi significativi nella preparazione dell'unità.
{% endcapture %}
{% include card.html title="Senior Developer, DevOps coach @ Infoservice s.r.l." subtitle="Key 2 Business s.r.l. | 10/12/2018 - presente" content=card10 footer="Skills: **ASP.NET MVC 5**, **Sql Server 2016**, **WebApi**, **UWP**, **WPF**, **WCF**, **Web Functions**, **Xamarin**, solution quality" %}

{% capture card9 %}
Per 2 mesi ho sviluppato alcune evolutive per la intranet di **Artigiancassa**, un portale SharePoint 2013 on premises.

Per 4 mesi ho progettato e sviluppato il portale **SharePoint Online** di Community ICT per **Ferrovie dello Stato Italiane**, con particolare enfasi nella **progettazione della soluzione e nella stesura di linee guida di sviluppo e standard di sviluppo e qualità del codice**.

Per 8 mesi ho sviluppato delle evolutive per il sistema di **streaming in diretta** di **UniCredit**. In particolare, le evolutive consistevano dell’implementazione in **C++08** all’interno del software di Encoding dello standard di streaming multimediale **HLS**, con produzione di frammenti audio-video in formato **MPEG2 Transport Stream** contenenti stream video in formato **H.264** e stream audio in formato **AAC+**.
...
Progettazione e sviluppo della Intranet **SharePoint Online** per **Ferrovie dello Stato Italiane**, con particolare enfasi nella **progettazione della soluzione e nella stesura di linee guida di sviluppo e standard di sviluppo e qualità del codice**, ricoprendo anche i ruoli di **Git Master**, **DevOps Architect** e **Technical Leader**. Nel progetto è coinvolta anche **Microsoft** con mansione di sviluppo affiancato con cui ho collaborato anche per l’implementazione delle funzionalità di **Continuous Integration**, **Continuous Deployment**, **Continuous Delivery**.
{% endcapture %}
